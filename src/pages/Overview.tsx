  ...
  return (
    <div className="p-6 space-y-6 max-w-[1400px]">
      ...
      <div className="grid grid-cols-6 gap-3">
        <MetricCard label="Total in DB" value={totalInDb.toLocaleString()} />
        <MetricCard label="Priority A/B" value={priorityAB} />
        <MetricCard label="Awaiting Approval" value={awaitingApproval} onClick={() => navigate('/pipeline')} />
        <MetricCard label="Letters Sent" value={lettersSent} />
        <MetricCard label="Interested" value={interested} />
        <MetricCard label="Added This Run" value="143" delta="+143" />
      </div>
      ...
      {awaitingApproval > 0 && (
        <div
          ...
          className="flex items-center gap-3 px-4 py-3 rounded-lg border border-tier-b/30 bg-tier-b/10 cursor-pointer hover:bg-tier-b/15 transition-colors"
        >
          <AlertTriangle className="h-4 w-4 text-tier-b shrink-0" />
          <span className="text-sm text-tier-b">
            ⚡ {awaitingApproval} companies waiting for approval — <span className="underline">open Pipeline to review</span>
          </span>
        </div>
      )}
      ...
      <div className="grid grid-cols-5 gap-4">
        ...
        <div className="col-span-3 bg-card border border-border rounded-lg p-4">
          <h3 className="text-sm font-medium text-foreground mb-4">Pipeline Funnel</h3>
      ...
      <div className="bg-card border border-border rounded-lg p-4">
        <h3 className="text-sm font-medium text-foreground mb-3">Top 10 Priority Targets</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border text-xs text-muted-foreground">
                ...
                <th className="text-left py-2 px-3">Director</th>
            ...
            <tbody>
              {top10.map(c => (
                <tr key={c.id} className="border-b border-border/50 hover:bg-accent/50 transition-colors">
                  ...
                  <td className="py-2 px-3 text-right font-mono-num text-muted-foreground">{c.age}</td>
                  <td className={`py-2 px-3 text-right font-mono-num font-bold ${getScoreColor(c.score)}`}>{c.score}</td>
                  ...
                  <td className="py-2 px-3 text-center"><StatusPill status={c.status} /></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="grid grid-cols-2 gap-4">
        {/* Sector Breakdown */}
        <div className="bg-card border border-border rounded-lg p-4">
          <h3 className="text-sm font-medium text-foreground mb-3">Sector Breakdown</h3>
        ...
        {/* Recent Activity */}
        <div className="bg-card border border-border rounded-lg p-4">
          <h3 className="text-sm font-medium text-foreground mb-3">Recent Activity (7 days)</h3>
          <ResponsiveContainer width="100%" height={260}>
